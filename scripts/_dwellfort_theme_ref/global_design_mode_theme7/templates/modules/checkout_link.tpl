...
<div data-position="{$pos}" class="{$pos} {if $text===false}icononly{/if} clearfix module checkout-link align-{$align}  {if $width}width width-{$width} width-valign-{$valign}{/if} {if !$width}valign-{$valign}{if $valign} valign{/if}{/if} {if $clear}clear-{$clear}{/if}">
	<div>
		<p><a href="/{$content.basket_link}">{if $text===false}{else}{$langs.Checkout}{/if} <span class="chekoutcount"></span></a></p>
	</div>
</div>
{/if}
{*
