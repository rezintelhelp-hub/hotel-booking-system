...
{
	"widget_info":{
		"title":"Shop Unleashed"
		,"title_info":"Enter a name for this instance of the Shop widget. This is just used for reference. Please note this widget assumes prices in Unleashed are not including tax."
		,"category":"setup"
	...
	"inner_templates":{
		"autoshop_unleashed": {
			...
			"editable_child_data": {"variant":"Variant"},
			"child_data":{
				"review":{
...
{if $smarty.get.app}
	...
	{/if}
{else}
<div class="unleashed-shop shop {if $singlepage}displaying-shop-product{else}displaying-shop-product{/if} {if $metadata.grid}shop_grid{/if}">
	...
	{if $singlepage}
		...
		{/if}
		{assign var="tagnames" value="`$tagnames``$all.name`"}
		{if $cattags!=""}
		...
		<div class='column twoCol first magic-heights'>
			...
			<div class="galleryWithThumbs withZoom" id="galId{$page.id}gallery">
				<div class="enlarge">
		...
		<div class='column twoCol last magic-heights'>
			...
			{if $page.meta.product_id!=""&&$page.meta.product_guid==""}
			{unleashed 
				action="getGuid"
				id=$page.meta.product_id
				api_id=$theme_vars_unleashed_id
				api_key=$theme_vars_unleashed_key
			}
			{page_metadata var=product_guid value=$guid pageid=$page.id}
			{/if}
			{if $page.meta.product_guid!=""}
				{unleashed 
					action="getProduct"
					guid=$page.meta.product_guid
					api_id=$theme_vars_unleashed_id
					api_key=$theme_vars_unleashed_key
				}
				...
				<p class="product-code">{$page.meta.product_id}<p>
				<p class="notes">{$product.Notes}</p>
				{/capture}
				{$smarty.capture.content_snippet}
				{add_page_search_data weight0=$product.Notes weight3=$page.meta.product_id weight4=$product.Description pageid=$page.id content_snippet=$smarty.capture.content_snippet}
				{assign var="tier" value=0}
				{assign var="discount" value=0}
				{assign var="tax_rate" value=$content.basket_tax_amount}
				{math assign="volume" equation="w * h * d" w=$product.Width h=$product.Height d=$product.Depth}
				{assign var="weight" value=$product.Weight}
				...
				{/if}
				{assign var="tier" value=$page.meta.default_price}
				...
				{if $content.logged_in_user}
					...
					{assign var="cursym" value="£"}
						{assign var="tax_rate" value=$widget_metadata.gbp_tax}
					{/if}
					{if $cursym=="USD"}
						{assign var="tax_rate" value=$widget_metadata.usd_tax}
					{/if}
					{if $cursym=="NZD"}
						{assign var="tax_rate" value=$widget_metadata.nzd_tax}
					{/if}
					{if $cursym=="AUD"}
						{assign var="tax_rate" value=$widget_metadata.aud_tax}
					{/if}
					...
					{if $cursym=="EUR"}
						{assign var="cursym" value="€"}
						{assign var="tax_rate" value=$widget_metadata.eur_tax}
					{/if}
				{else}
					...
					{if $content.currency_code|strtolower!=$cursymout|strtolower}
						{assign var="tax_rate" value=$widget_metadata.eur_tax}
					{/if}
					{/if}
				{/if}
				...
				{/strip}{if $discount>0}<strike>{$cursym}{assign var="origprice" value=$price}{math equation="(1 - x) * y" x=$discount y=$price assign=price}{if $tax_rate!=0}{math equation="x * y / 100 + y" x=$tax_rate y=$origprice assign="incl"}{else}{assign var="incl" value=$origprice}{/if}{$incl|string_format:'%.2f'} </strike> {strip}
				...
				<form action="" method="post" class="append_url">
				...
				{if $smarty.request.addtobasket}
					{assign var="cur" value=""}
					{assign var="custom_tax" value=""}
					{if $smarty.cookies.output_cursymv3}
					{assign var="cur" value=$smarty.cookies.output_cursymv3}
					{assign var="custom_tax" value=$tax_rate}
					{/if}
					{if $content.logged_in_user}
					{assign var="cur" value=$cursymout}
					{assign var="custom_tax" value=$tax_rate}
					{/if}
					{assign var="product_name" value="`$product.ProductDescription` (`$page.meta.product_id`)"}
					{* add tags to order line for testing for coupon elligibility *}
					{add_to_basket
						name=$product_name
						price=$price
						original_price=$origprice
						custom_code=$page.meta.product_id
		...
		{if $page.page_child_data.review|count}
		<h2>Reviews</h2>
		{/if}
		...
		{if $smarty.get.category||!$smarty.get.category}
			{if $smarty.get.astag}
				{foreach from=$flat_publictags item=tag}
					{if $tag.name|css_safe==$smarty.get.category} 
						<p class="Icon_Info">You are shopping for <strong>{$tag.name}</strong></p>
					{/if}
				{/foreach}
			{else}
				{foreach from=$flat_cats item=tag}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$(".append_url").append("<input type='hidden' name='url' value='"+window.location.href+"' />");
		setShopMenus();
		$(".depth-2").prev(".depth-1").append("<span></span>");
		$(".depth-3").prev(".depth-2").append("<span></span>");
		$(".shop-categories li span").click(function(){
			$(this).parent().toggleClass("current");
			setShopMenus();
		});
		$(".showcats a").unbind().click(function(){
		 	$("#sidebarNav").toggle();
			return false;
		});
		moduleHeights();
