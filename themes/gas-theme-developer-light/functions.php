...
function developer_developer_scripts() {
    ...
    $font_names = array(
        ...
        'rokkitt'           => 'Rokkitt:wght@300;400;500;600;700',
...
function developer_get_api_settings() {
    ...
    $result = array(
        ...
        // Global typography colours (Steve 2026-08-15). Front-page.php uses
        // these as fallbacks for per-section title/subtitle/text colours so
        // an unset section defaults to the global — operator per-section
        // colour still wins when explicitly set.
        ...
        // Gallery body — image URLs + display settings. Plugin reads these in
        // gas-booking.php:9532 (images-json) and the legacy image-N fallbacks.
        'page_gallery_images_json' => ($website['page-gallery'] ?? [])['images-json'] ?? '',
        'page_gallery_columns'    => ($website['page-gallery'] ?? [])['columns']    ?? '4',
        'page_gallery_style'      => ($website['page-gallery'] ?? [])['style']      ?? 'grid',
        'page_gallery_bg'         => ($website['page-gallery'] ?? [])['bg']         ?? '#ffffff',
        'page_gallery_lightbox'   => ($website['page-gallery'] ?? [])['lightbox']   ?? true,
        'page_gallery_image_1'  => ($website['page-gallery'] ?? [])['image-1-url']  ?? '',
        'page_gallery_image_2'  => ($website['page-gallery'] ?? [])['image-2-url']  ?? '',
        'page_gallery_image_3'  => ($website['page-gallery'] ?? [])['image-3-url']  ?? '',
        'page_gallery_image_4'  => ($website['page-gallery'] ?? [])['image-4-url']  ?? '',
        'page_gallery_image_5'  => ($website['page-gallery'] ?? [])['image-5-url']  ?? '',
        'page_gallery_image_6'  => ($website['page-gallery'] ?? [])['image-6-url']  ?? '',
        'page_gallery_image_7'  => ($website['page-gallery'] ?? [])['image-7-url']  ?? '',
        'page_gallery_image_8'  => ($website['page-gallery'] ?? [])['image-8-url']  ?? '',
        'page_gallery_image_9'  => ($website['page-gallery'] ?? [])['image-9-url']  ?? '',
        'page_gallery_image_10' => ($website['page-gallery'] ?? [])['image-10-url'] ?? '',
        'page_gallery_image_11' => ($website['page-gallery'] ?? [])['image-11-url'] ?? '',
        'page_gallery_image_12' => ($website['page-gallery'] ?? [])['image-12-url'] ?? '',
        ...
        'page_portal_hero_enabled' => ($website['page-portal'] ?? [])['hero-enabled'] ?? true,
        'page_portal_hero_image' => ($website['page-portal'] ?? [])['hero-image-url'] ?? '',
        'page_portal_header_bg' => ($website['page-portal'] ?? [])['header-bg'] ?? '#1e293b',
        'page_portal_header_text' => ($website['page-portal'] ?? [])['header-text'] ?? '#ffffff',
        ...
        'page_contact_directions_mode' => $website_page_contact['directions-mode'] ?? 'link',
        'page_contact_directions_extra' => developer_get_ml_value($website_page_contact, 'directions_extra', $lang) ?: '',
        'page_contact_extra_title' => developer_get_ml_value($website_page_contact, 'extra_title', $lang) ?: '',
        'page_contact_extra_content' => developer_get_ml_value($website_page_contact, 'extra_content', $lang) ?: '',
        ...
        // Steve 2026-08-15 — Hero Badge now has proper Web Builder fields:
        // Badge Text (badge-text-copy-{lang}), Badge Logo (badge-image-url),
        // Badge Link (badge-link) with new-tab toggle (badge-new-tab). The
        // legacy button_text / button-link fallbacks stay for old sites that
        // never had dedicated badge fields — theme still renders correctly.
        ...
        'hero_search_labels' => $website_hero['search-labels'] ?? ($website_hero['search']['labels'] ?? null),
        ...
        'services_item_1_link' => $website_services['item-1-link'] ?? '',
        ...
        'services_item_2_link' => $website_services['item-2-link'] ?? '',
        ...
        'services_item_3_link' => $website_services['item-3-link'] ?? '',
        ...
        'services_item_4_link' => $website_services['item-4-link'] ?? '',
        ...
        'services_item_5_link' => $website_services['item-5-link'] ?? '',
        ...
        'services_item_6_link' => $website_services['item-6-link'] ?? '',
        ...
        'services_item_7_link' => $website_services['item-7-link'] ?? '',
        ...
        'services_item_8_link' => $website_services['item-8-link'] ?? '',
        ...
        // Wrap Section — magazine-style block: body text wraps around a floated
        // image or YouTube/Vimeo video. Optional callout card overlay (e.g. lead-
        // magnet "free guide" CTA). Added to the editor 2026-05-18 (c041ec44);
        // theme render added 2026-06-02. Position defaults to 4 (between Intro=3
        // and Featured=5). Hidden when enabled=false or text-{lang} is empty.
        ...
        // Card title colour — new setting 2026-09-15. Falls back to the
        // section title colour so every existing site keeps its current
        // look; only sites that explicitly set it get the split.
        ...
        // display_mode default — operator's explicit choice wins; otherwise
        // infer from what's actually populated so legacy sites that uploaded
        // images before the dropdown shipped continue to render correctly.
        'usp_item_1_display_mode' => $website_usp['item-1-display-mode'] ?? 'icon',
        'usp_item_1_link' => $website_usp['item-1-link'] ?? '',
        ...
        'usp_item_2_display_mode' => $website_usp['item-2-display-mode'] ?? 'icon',
        'usp_item_2_link' => $website_usp['item-2-link'] ?? '',
        ...
        'usp_item_3_display_mode' => $website_usp['item-3-display-mode'] ?? 'icon',
        'usp_item_3_link' => $website_usp['item-3-link'] ?? '',
        ...
        'usp_item_4_display_mode' => $website_usp['item-4-display-mode'] ?? 'icon',
        'usp_item_4_link' => $website_usp['item-4-link'] ?? '',
        ...
        'usp_item_5_display_mode' => $website_usp['item-5-display-mode'] ?? 'icon',
        'usp_item_5_link' => $website_usp['item-5-link'] ?? '',
        ...
        'usp_item_6_display_mode' => $website_usp['item-6-display-mode'] ?? 'icon',
        'usp_item_6_link' => $website_usp['item-6-link'] ?? '',
        ...
        // NOTE: 'rooms_show_map' is set above at line 3079 from page-rooms.show-map.
        // Do NOT redefine it here from pro-settings — duplicate keys silently overwrite
        // and a missing pro-settings row would force show_map back to the 'true' string,
        // ignoring the operator's page-rooms toggle. (Moon River /book-now/ regression
        // 2026-06-24.)
        ...
        'image_row_1_description_font_size' => ($website['image-row-1'] ?? array())['description-font-size'] ?? '1rem',
        'image_row_1_description_max_width' => ($website['image-row-1'] ?? array())['description-max-width'] ?? '800',
        'image_row_1_heading_color' => ($website['image-row-1'] ?? array())['heading-color'] ?? null,
        ...
        'image_row_1_image_4' => ($website['image-row-1'] ?? array())['image-4'] ?? '',
        'image_row_1_title_4' => developer_get_ml_value($website['image-row-1'] ?? array(), 'title-4', $lang),
        'image_row_1_text_4' => developer_get_ml_value($website['image-row-1'] ?? array(), 'text-4', $lang),
        'image_row_1_btn_text_4' => developer_get_ml_value($website['image-row-1'] ?? array(), 'btn-text-4', $lang),
        'image_row_1_btn_link_4' => ($website['image-row-1'] ?? array())['btn-link-4'] ?? '',
        'image_row_1_card_bg_4' => ($website['image-row-1'] ?? array())['card-bg-4'] ?? '',
        ...
        'image_row_2_description_font_size' => ($website['image-row-2'] ?? array())['description-font-size'] ?? '1rem',
        'image_row_2_description_max_width' => ($website['image-row-2'] ?? array())['description-max-width'] ?? '800',
        'image_row_2_heading_color' => ($website['image-row-2'] ?? array())['heading-color'] ?? null,
        ...
        'image_row_2_image_4' => ($website['image-row-2'] ?? array())['image-4'] ?? '',
        'image_row_2_title_4' => developer_get_ml_value($website['image-row-2'] ?? array(), 'title-4', $lang),
        'image_row_2_text_4' => developer_get_ml_value($website['image-row-2'] ?? array(), 'text-4', $lang),
        'image_row_2_btn_text_4' => developer_get_ml_value($website['image-row-2'] ?? array(), 'btn-text-4', $lang),
        'image_row_2_btn_link_4' => ($website['image-row-2'] ?? array())['btn-link-4'] ?? '',
        'image_row_2_card_bg_4' => ($website['image-row-2'] ?? array())['card-bg-4'] ?? '',
        ...
        'image_row_3_description_font_size' => ($website['image-row-3'] ?? array())['description-font-size'] ?? '1rem',
        'image_row_3_description_max_width' => ($website['image-row-3'] ?? array())['description-max-width'] ?? '800',
        'image_row_3_heading_color' => ($website['image-row-3'] ?? array())['heading-color'] ?? null,
        ...
        'image_row_3_image_4' => ($website['image-row-3'] ?? array())['image-4'] ?? '',
        'image_row_3_title_4' => developer_get_ml_value($website['image-row-3'] ?? array(), 'title-4', $lang),
        'image_row_3_text_4' => developer_get_ml_value($website['image-row-3'] ?? array(), 'text-4', $lang),
        'image_row_3_btn_text_4' => developer_get_ml_value($website['image-row-3'] ?? array(), 'btn-text-4', $lang),
        'image_row_3_btn_link_4' => ($website['image-row-3'] ?? array())['btn-link-4'] ?? '',
        'image_row_3_card_bg_4' => ($website['image-row-3'] ?? array())['card-bg-4'] ?? '',
        ...
        'image_row_4_heading_color' => ($website['image-row-4'] ?? array())['heading-color'] ?? null,
        ...
        'image_row_4_description_font_size' => ($website['image-row-4'] ?? array())['description-font-size'] ?? '1rem',
        'image_row_4_description_max_width' => ($website['image-row-4'] ?? array())['description-max-width'] ?? '800',
        ...
        'image_row_4_image_4' => ($website['image-row-4'] ?? array())['image-4'] ?? '',
        'image_row_4_title_4' => developer_get_ml_value($website['image-row-4'] ?? array(), 'title-4', $lang),
        'image_row_4_text_4' => developer_get_ml_value($website['image-row-4'] ?? array(), 'text-4', $lang),
        'image_row_4_btn_text_4' => developer_get_ml_value($website['image-row-4'] ?? array(), 'btn-text-4', $lang),
        'image_row_4_btn_link_4' => ($website['image-row-4'] ?? array())['btn-link-4'] ?? '',
        'image_row_4_card_bg_4' => ($website['image-row-4'] ?? array())['card-bg-4'] ?? '',
...
function developer_developer_custom_css() {
    ...
    // Map the Web Builder's btn-size select to actual padding + font-size.
    // Affects both the theme's .developer-btn AND the gas-booking plugin's
    // .gas-view-btn ("View & Book" on room cards) so the Styles section's
    // Button Size control behaves consistently everywhere on the public site.
    $btn_size_map = [
    ...
    // in Web Builder → Header → CTA Button Style.
    ...
    echo '<style id="developer-custom-css">
        ...
        /* Button Size — Web Builder Styles section, three-tier select.
           Steve 2026-07-28: was only wired to 3 classes; extended to
           every CTA button class on the public site (theme + plugin)
           so Small actually shrinks buttons everywhere consistently.
           Excludes utility buttons (copy / filter / tab) which have
           their own tight sizing and must not follow marketing CTAs. */
        .developer-btn,
        .developer-nav-cta,
        .gas-view-btn,
        .gas-row-view-btn,
        ...
        /* 2026-07-22 Steve — long-standing bug: heading/body font from Web
           ...
           Root causes:
             1. Form inputs don\'t inherit font-family by default → stayed browser sans
             2. Plugin CSS had hardcoded system stack on .gas-room-widget → overrode body
           Body + h1..h6 rules in the static style.css already read the CSS
           variables. Adding explicit inherit for form fields + explicit font
           on the booking-widget selectors (belt-and-braces alongside the
           plugin CSS fix) so future shortcodes without their own font work too. */
