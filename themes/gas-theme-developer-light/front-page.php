...
// Steve 2026-08-15 — Global typography colours from Web Builder → Styles →
// Typography. Every per-section title/subtitle/text fallback below falls
// through to these first, and the hardcoded #1e293b / #64748b defaults
// only fire if BOTH the section-specific value and the global are unset.
// Operator per-section colour (when explicitly set) still wins first.
...
<section class="developer-hero" <?php if ($hero_background_type === 'slider') : ?>data-slider-duration="<?php echo esc_attr($hero_slider_duration); ?>" data-slider-transition="<?php echo esc_attr($hero_slider_transition); ?>"<?php endif; ?>>
    ...
    <div class="developer-hero-content<?php echo $hero_align_class; ?>">
        ...
        <?php
        // Steve 2026-08-15 — Trust Badges hide toggle. Default was implicit-on
        // (render whenever any of the 3 texts had content). Now honours the
        // Show Trust Badges checkbox in Web Builder: off → nothing renders
        // regardless of the trust-N field values.
        $hero_show_trust = !isset($api['hero_show_trust']) || $api['hero_show_trust'];
        if ($hero_show_trust && ($hero_badge_1 || $hero_badge_2 || $hero_badge_3)) : ?>
...
// embedded YouTube/Vimeo video. Optional callout card. Only renders when
// enabled and there's body text in the active language.
...
if ($wrap_enabled && $wrap_enabled !== 'false' && !empty($wrap_text)) :
    ...
    // theme was never updated. Fallback uses !empty because functions.php
    // defaults wrap_media_url to '' when the DB key is missing, and PHP's
    // ?? only fires on null, not empty string. Falls through to the
    // consolidated wrap_media_image_url when the legacy key is empty.
    $wrap_media_url = !empty($api['wrap_media_url']) ? $api['wrap_media_url'] : ($api['wrap_media_image_url'] ?? '');
...
<section class="developer-section developer-wrap" style="background: <?php echo esc_attr($wrap_bg); ?>; color: <?php echo esc_attr($wrap_text_color); ?>;">
    <div class="developer-container">
        ...
        <div class="developer-wrap-content" style="overflow: hidden;">
            ...
            // at the same width. The entire column floats to the chosen side; body
            // text wraps around BOTH (per the Web Builder hint "Card (stacks under
            // the media) — Renders directly below the media at the same width").
...
<section class="developer-section developer-featured" style="background-color: <?php echo esc_attr($featured_bg); ?>;">
    <div class="developer-container">
        <div class="developer-section-header">
            ...
            <p style="color: <?php echo esc_attr($featured_subtitle_color); ?>;<?php echo esc_attr($featured_subtitle_max_width_css); ?>"><?php echo nl2br(wp_kses_post($featured_subtitle)); ?></p>
        </div>
    ...
    <div class="developer-container">
        ...
        <div class="developer-services-grid">
            ...
            foreach ($services_items as $item) :
                // Skip cards that are effectively empty — no image / icon,
                // no title, and no meaningful text (after stripping <br>
                // and whitespace). Dwellfort 2026-08-16 had an 8th card
                // with only "<br />" that rendered as a blank tile.
...
<section class="developer-section developer-reviews" style="background: <?php echo esc_attr($reviews_bg); ?>; color: <?php echo esc_attr($reviews_text_color); ?>;">
    <div class="developer-container">
        ...
        <div style="text-align: center; margin-top: 2rem;">
            <a href="<?php echo esc_url($btn_href); ?>"<?php echo $btn_target; ?> style="display: inline-block; padding: 12px 32px; background: <?php echo esc_attr($btn_color); ?>; color: <?php echo esc_attr($btn_text_color); ?>; text-decoration: none; border-radius: <?php echo esc_attr($btn_radius); ?>px; font-weight: 600; border: 2px solid <?php echo esc_attr($btn_color); ?>; transition: all 0.3s ease;"><?php echo esc_html($btn_text); ?></a>
        </div>
        <?php endif; ?>
    </div>
</section>
        ...
        <div class="developer-container">
            ...
            <div class="developer-image-row-grid" style="display: grid; grid-template-columns: repeat(<?php echo $ir_cols; ?>, 1fr); gap: 24px;">
                ...
                <div style="text-align: <?php echo esc_attr($ir_align); ?>; <?php echo $card_bg_style; ?>">
                    ...
                    ?>
                        <a href="<?php echo esc_url($ir_href); ?>"<?php echo $ir_target; ?> class="developer-btn" style="display: inline-block; margin-top: 8px; padding: 12px 32px; border-radius: <?php echo esc_attr($btn_radius); ?>px; text-decoration: none; font-weight: 600;"><?php echo esc_html($iri['btn_text']); ?></a>
                    <?php endif; ?>
                </div>
                <?php endforeach; ?>
            </div>
            ...
            ?>
            <div style="text-align: center; margin-top: 2rem;">
                <a href="<?php echo esc_url($ir_row_href); ?>"<?php echo $ir_row_target; ?> class="developer-btn" style="display: inline-block; padding: 14px 36px; border-radius: <?php echo esc_attr($btn_radius); ?>px; text-decoration: none; font-weight: 600;"><?php echo esc_html($ir_row_btn_text); ?></a>
            </div>
            <?php endif; ?>
        </div>
    </section>
