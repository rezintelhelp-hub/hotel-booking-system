...
function gas_render_page_sections($page_slug, $primary_color = '#2563eb') {
    ...
    // Kept empty when unset or default #ffffff so we don't force a colour
    // on pages the operator never touched. When set, drives the spacer
    // colour and wraps all sections so gaps/below-last-section show it too.
    // Fix 2026-09-14 — Web Builder colour picker was silently ignored on
    // any page rendered via Section Builder.
    $__gas_page_bg = '';
    ...
    // Button palette from Styles & Fonts. CTAs across section builder inherit
    // these unless overridden per-item, so operators manage button colour in
    // ONE place (Styles) instead of hunting per-section. Falls back to primary
    // colour + white when unset to keep older sites visually consistent.
    // Fix 2026-09-14 — book-jet /about/ Contact Us button rendered #c3d5e1
    // (primary_color) even though Styles → Buttons was #444444.
    ...
    // Wrap all output in a page-bg div when operator set a colour, so gaps
    // between sections and below-last-section space render the chosen colour.
    if ($__gas_page_bg !== '') {
    ...
    if (!$hero_enabled_ps) {
        ...
        $__ps_sbg  = defined('GAS_SPARK_HEADER_SPACER_BG') ? GAS_SPARK_HEADER_SPACER_BG : ($__gas_page_bg !== '' ? $__gas_page_bg : $__ps_spacer_bg($sections));
    ...
    foreach ($sections as $section) {
        ...
        switch ($type) {
                ...
                <section<?php echo $id_attr; ?> class="gas-ps-section gas-ps-text" style="padding: 40px 24px; background: <?php echo $bg_col ? esc_attr($bg_col) : '#fff'; ?>;">
                    <div style="max-width: <?php echo $max_w; ?>; margin: 0 auto;">
                        ...
                        <?php if ($cta_text && $cta_link) : ?>
                            <div style="text-align: <?php echo esc_attr($text_align); ?>; margin-top: 24px;">
                ...
                <section<?php echo $id_attr; ?> class="gas-ps-section gas-ps-imgtext" style="padding: 40px 24px; background: <?php echo $bg_col ? esc_attr($bg_col) : '#fff'; ?>;">
                    <div class="gas-ps-imgtext-grid" style="max-width: 1100px; margin: 0 auto; display: grid; grid-template-columns: <?php echo $grid_cols; ?>; gap: 32px; align-items: center;">
                        ...
                        <div style="<?php echo $txt_order; ?>">
                            ...
                            ?>
                            <a href="<?php echo esc_url($cta_href); ?>"<?php echo $cta_target; ?> style="display: inline-block; margin-top: 16px; background: <?php echo esc_attr($btn_primary_bg); ?>; color: <?php echo esc_attr($btn_primary_text); ?>; padding: 12px 32px; border-radius: <?php echo esc_attr($btn_radius); ?>px; text-decoration: none; font-weight: 600;"><?php echo esc_html($cta_text); ?></a>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>
            ...
            case 'media_wrap':
                ...
                // back to the single-image behaviour (back-compat for every
                // existing site — they all default to unticked / single image).
                ...
                <section<?php echo $id_attr; ?> class="gas-ps-section gas-ps-videos" style="padding: 40px 24px; background: <?php echo $bg_col ? esc_attr($bg_col) : '#fff'; ?>;">
                    <div style="max-width: <?php echo esc_attr($max_w); ?>; margin: 0 auto;">
                        ...
                        <div style="display: grid; grid-template-columns: repeat(<?php echo $vid_cols; ?>, 1fr); gap: 24px;">
                            ...
                            ?>
                            <div>
                                <div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; border-radius: <?php echo esc_attr($card_radius); ?>px; box-shadow: 0 4px 20px rgba(0,0,0,0.1);">
                                    <iframe src="<?php echo esc_url($embed_url); ?>" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: 0; border-radius: <?php echo esc_attr($card_radius); ?>px;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
                                </div>
                                <?php if ($vid_title) : ?>
                    ...
                    <div style="max-width: <?php echo esc_attr($max_w); ?>; margin: 0 auto;">
                        ...
                        ?>
                            ...
                            <div style="position: relative;">
                                ...
                                <div id="<?php echo esc_attr($car_id); ?>" class="gas-ps-gallery-carousel" style="display: flex; gap: 16px; overflow-x: auto; scroll-snap-type: x proximity; scroll-behavior: smooth; padding-bottom: 12px; -webkit-overflow-scrolling: touch; scrollbar-width: thin;">
                                    <?php foreach ($images as $img) :
                                        $src = is_array($img) ? ($img['url'] ?? $img['src'] ?? '') : $img;
                                        $alt = is_array($img) ? ($img['alt'] ?? '') : '';
                                    ?>
                                        <img src="<?php echo esc_url($src); ?>" alt="<?php echo esc_attr($alt); ?>" loading="lazy" style="flex: 0 0 calc((100% - 32px) / 3); min-width: 220px; height: 280px; object-fit: cover; scroll-snap-align: start; border-radius: <?php echo esc_attr($card_radius); ?>px;">
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        <?php else : ?>
                    ...
                    <div style="max-width: <?php echo esc_attr($max_w); ?>; margin: 0 auto;">
                        ...
                        <div class="gas-ps-cards-grid" style="display: grid; grid-template-columns: repeat(<?php echo $card_cols; ?>, 1fr); gap: 24px;">
                            <?php foreach ($cards as $card) : ?>
                                ...
                                $card_body_color = !empty($card['card_text_color']) ? $card['card_text_color'] : '#475569';
                                $cta_pad = $cta_size === 'lg' ? '16px 40px' : ($cta_size === 'md' ? '12px 32px' : '8px 20px');
                                $cta_font = $cta_size === 'lg' ? '1.05rem' : ($cta_size === 'md' ? '0.95rem' : '0.85rem');
                                ?>
                                <div style="background: <?php echo esc_attr($card_panel_bg); ?>; border-radius: <?php echo esc_attr($card_radius); ?>px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.06);">
                                    ...
                                    <div style="padding: 24px;">
                                        <?php if (!empty($card_title)) : ?><h3 style="font-size: 1.2rem; font-weight: 600; color: <?php echo esc_attr($card_title_color); ?>; margin: 0 0 8px;"><?php echo esc_html($card_title); ?></h3><?php endif; ?>
                        ...
                        // The admin UI (gas-admin.html:42370) has always
                        // saved button_text + button_link at the cards
                        // section level; theme never emitted it. Steve
                        // 2026-09-11.
                        ...
                        <div id="<?php echo esc_attr($form_uid); ?>-wrap" style="background: <?php echo esc_attr($card_bg); ?>; border-radius: <?php echo esc_attr($card_radius_form); ?>px; padding: 24px; border: 1px solid #e5e7eb;">
                            <form id="<?php echo esc_attr($form_uid); ?>" method="POST" enctype="multipart/form-data" action="<?php echo esc_url($form_api . '/api/public/form-submit'); ?>">
                                ...
                                foreach ($fields as $fi => $f) {
                                    ...
                                    // Slugify label for the input name so the
                                    // recipient email reads "Email: foo" instead
                                    // of "field_2: foo". Collisions / empty
                    ...
                    <section<?php echo $id_attr; ?> class="gas-ps-section gas-ps-reviews" style="padding: 60px 24px; background: <?php echo $rev_section_bg; ?>;">
                        <div style="max-width: 1200px; margin: 0 auto;">
                            ...
                            <?php if ($rev_layout === 'slider') :
                                // Homepage-replica slider (mirrors front-page.php's repuso/hostaway
                                // renderer). 4 cards @ 1280+, 3 @ 1024, 2 @ 768, 1 mobile. Auto-
                                // advances every 5s. Prev/next arrows in the star colour. Fixed
                                // 298px card height so uneven text lengths don't jitter the row.
                                // Steve 2026-09-11 — SB reviews was rendering the full list layout;
                                // slider is what the operator sees on the homepage.
                                ?>
                                <style>
