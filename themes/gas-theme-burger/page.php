...
// Terms page — delegate to dedicated template IF that template exists.
// Without the file_exists() guard, missing template throws fatal on every
// page with one of these slugs (Hebden /terms/, /privacy/ etc.). Falls
// through to the normal page render below when the template isn't there.
...
if (in_array($page_slug_early, array('terms', 'terms-and-conditions', 'terms-of-service'))) {
    $tpl = get_template_directory() . '/template-terms.php';
    if (file_exists($tpl)) { require $tpl; return; }
}

// Privacy page — same defensive guard.
if (in_array($page_slug_early, array('privacy', 'privacy-policy'))) {
    $tpl = get_template_directory() . '/template-privacy.php';
    if (file_exists($tpl)) { require $tpl; return; }
}
...
if (in_array($page_slug, array('properties', 'apartments')) && shortcode_exists('gas_properties')) {
    ...
    // Rooms page hero settings from Web Builder
    $rooms_title = $api['page_rooms_title'] ?? 'Book Your Stay';
    $rooms_subtitle = $api['page_rooms_subtitle'] ?? '';
    $rooms_hero_image = $api['page_rooms_hero_image'] ?? '';
    $rooms_header_bg = $api['page_rooms_header_bg'] ?? '#1e293b';
    $rooms_header_text = $api['page_rooms_header_text'] ?? '#ffffff';
    $rooms_hero_enabled_val = $api['page_rooms_hero_enabled'] ?? false;
    $rooms_hero_enabled = !($rooms_hero_enabled_val === false || $rooms_hero_enabled_val === 'false' || $rooms_hero_enabled_val === '0' || $rooms_hero_enabled_val === 0);

    // Parse overlay color
    $r_hex = $rooms_header_bg;
    if (strlen($r_hex) === 7 && $r_hex[0] === '#') {
        $r_r = hexdec(substr($r_hex, 1, 2)); $r_g = hexdec(substr($r_hex, 3, 2)); $r_b = hexdec(substr($r_hex, 5, 2));
    } else { $r_r = 30; $r_g = 41; $r_b = 59; }

    if ($rooms_hero_enabled) : ?>
    <section class="developer-page-hero" style="position:relative; min-height:<?php echo $rooms_hero_image ? '350px' : '250px'; ?>; height:<?php echo $rooms_hero_image ? '50vh' : '35vh'; ?>; display:flex; align-items:center; justify-content:center; overflow:hidden;">
        <?php if ($rooms_hero_image) : ?>
            <div style="position:absolute; inset:0; background-image:url('<?php echo esc_url($rooms_hero_image); ?>'); background-size:cover; background-position:center;"></div>
        <?php else : ?>
            <div style="position:absolute; inset:0; background:<?php echo esc_attr($rooms_header_bg); ?>;"></div>
        <?php endif; ?>
        <div style="position:absolute; inset:0; background:rgba(<?php echo "$r_r,$r_g,$r_b"; ?>,<?php echo $rooms_hero_image ? '0.5' : '0.1'; ?>);"></div>
        ...
        <div style="position:relative; z-index:2; text-align:center; padding:80px 24px 0; max-width:900px;">
            <h1 style="font-size:clamp(2.5rem,5vw,4rem); font-weight:700; color:<?php echo esc_attr($rooms_header_text); ?>; margin:0 0 16px; text-shadow:0 2px 20px rgba(0,0,0,0.3);"><?php echo esc_html($rooms_title); ?></h1>
            <?php if ($rooms_subtitle) : ?>
                <p style="font-size:clamp(1.1rem,2vw,1.35rem); color:<?php echo esc_attr($rooms_header_text); ?>; opacity:0.9; margin:0 auto; max-width:600px; text-shadow:0 1px 10px rgba(0,0,0,0.2);"><?php echo esc_html($rooms_subtitle); ?></p>
            <?php endif; ?>
        </div>
    </section>
    <?php else : ?>
    <?php endif; ?>
    <style>.developer-book-now-page .gas-rooms-page-wrapper > .gas-date-filter:first-child { margin-top:0 !important; }</style>
    <div class="developer-book-now-page" style="padding-top:0;">
...
// Pro Builder pages — render block content only, skip special page logic
$pb_content = get_post_field("post_content", get_the_ID());
if (strpos($pb_content, "wp-block-cover") !== false || strpos($pb_content, "wp-block-columns") !== false || strpos($pb_content, "wp-block-group") !== false) {
    get_header();
    echo "<main id=\"main\" class=\"site-main\" style=\"margin-top:0;\">";
    echo "<div class=\"gas-burger-content\">";
...
// Non-special pages (Pro Builder): skip hero entirely, render content in gas-burger-content
if (empty($special_page)) : ?>
    <div class="gas-burger-content">
...
if ($special_page === 'about') {
    ...
    $hero_enabled = !($hero_val === false || $hero_val === 'false' || $hero_val === '0' || $hero_val === 0);
} elseif ($special_page === 'properties') {
    $hero_hidden = true;
    $hero_enabled = false;
}
?>
...
<?php elseif (!empty($hero_hidden)) : ?>
<!-- No hero, just top padding -->
