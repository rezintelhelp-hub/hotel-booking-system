...
function developer_get_api_settings() {
    ...
    $result = array(
        ...
        'page_gallery_hero_enabled' => $website_page_gallery['hero-enabled'] ?? true,
        'page_gallery_hero_image'   => $website_page_gallery['hero-image-url'] ?? '',
        'page_gallery_header_bg'    => $website_page_gallery['header-bg'] ?? '#1e293b',
        'page_gallery_header_text'  => $website_page_gallery['header-text'] ?? '#ffffff',
        'page_gallery_images_json'  => $website_page_gallery['images-json'] ?? '',
        'page_gallery_columns'      => $website_page_gallery['columns']    ?? '4',
        'page_gallery_style'        => $website_page_gallery['style']      ?? 'grid',
        'page_gallery_bg'           => $website_page_gallery['bg']         ?? '#ffffff',
        'page_gallery_lightbox'     => $website_page_gallery['lightbox']   ?? true,
        'page_gallery_image_1'  => $website_page_gallery['image-1-url']  ?? '',
        'page_gallery_image_2'  => $website_page_gallery['image-2-url']  ?? '',
        'page_gallery_image_3'  => $website_page_gallery['image-3-url']  ?? '',
        'page_gallery_image_4'  => $website_page_gallery['image-4-url']  ?? '',
        'page_gallery_image_5'  => $website_page_gallery['image-5-url']  ?? '',
        'page_gallery_image_6'  => $website_page_gallery['image-6-url']  ?? '',
        'page_gallery_image_7'  => $website_page_gallery['image-7-url']  ?? '',
        'page_gallery_image_8'  => $website_page_gallery['image-8-url']  ?? '',
        'page_gallery_image_9'  => $website_page_gallery['image-9-url']  ?? '',
        'page_gallery_image_10' => $website_page_gallery['image-10-url'] ?? '',
        'page_gallery_image_11' => $website_page_gallery['image-11-url'] ?? '',
        'page_gallery_image_12' => $website_page_gallery['image-12-url'] ?? '',
        ...
        // display_mode default — explicit operator choice wins; else infer
        // from what's populated (legacy back-compat).
        ...
        // NOTE: 'rooms_show_map' is set above from page-rooms.show-map. Do NOT
        // redefine it here from pro-settings — duplicate keys silently overwrite
        // and a missing pro-settings row would force show_map back to the 'true'
        // string, ignoring the operator's page-rooms toggle. (Moon River regression
        // 2026-06-24, mirrored from light theme.)
...
function developer_developer_custom_css() {
    ...
    // No-hero pages need a SOLID background — zero-alpha fallback to white.
    // See gas-theme-developer-light/functions.php for full rationale.
    ...
    // override of .developer-header-inner height for a fatter/thinner row.
    ...
    if ($header_transparent) {
        ...
        /* No-hero pages (book-now, regions, custom pages without a hero block):
           force the same SOLID styles the header uses when scrolled so the
           operator base colours (often white logo + dark nav, chosen for
           over-hero use) do not clash with plain page backgrounds. Same fix
           lives in gas-theme-developer-light/functions.php. */
    ...
    // Mirror hero search radius onto /book-now/ filter bar — match dev-light.
 ...
 * Fires on template_redirect so it beats get_header() — no "headers already
 * sent" and no partial page render before the bounce. Steve 2026-08-14 —
 * Atlantis /privacy/ was showing GAS default template because template-*.php
 * never consumed the use-external flag despite it existing in $api.
 */
