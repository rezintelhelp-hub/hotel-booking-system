...
class GAS_Blog {
    ...
    public function blog_shortcode($atts) {
        ...
        $cr = intval($c['card_radius'] ?? 12).'px';
        foreach ($posts as $p) { $h .= '<article class="gas-sc-bc" style="background:'.$c['card_bg'].';border-radius:'.$cr.';overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);transition:all .2s"><a href="'.esc_url(home_url('/blog/'.$p['slug'])).'" style="text-decoration:none;color:inherit;display:block">'; if (!empty($p['featured_image_url'])) $h .= '<img src="'.esc_url($p['featured_image_url']).'" style="width:100%;height:200px;object-fit:cover">'; $h .= '<div style="padding:12px 14px"><h3 style="margin:0 0 6px;font-size:1.1rem;color:'.$c['text'].'">'.esc_html($p['title']).'</h3>'; if (!empty($p['excerpt'])) $h .= '<p style="color:'.$c['text_secondary'].';margin:0;font-size:0.9rem">'.esc_html(wp_trim_words($p['excerpt'],20)).'</p>'; $h .= '</div></a></article>'; }
        return $h.'</div>';
    }
