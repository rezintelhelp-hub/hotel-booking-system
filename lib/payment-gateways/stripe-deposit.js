...
async function block({ stripe, booking, amount, currency, returnUrl, idempotencyKey }) {
  ...
  try {
    const intent = await stripe.paymentIntents.create({
      ...
      confirm: true,
      off_session: true,
      // Don't pass receipt_email — Stripe validates it and 'null' is rejected
      // as an invalid address. Omitting the key entirely tells Stripe not to
      // send a receipt. We email the guest ourselves via the sendEmail wrapper.
