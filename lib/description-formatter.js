...
function removeMangledEmoji(text) {
  ...
  return text
    ...
    .replace(/(^|[a-zA-Z\d.,!]|\s)\?(\s+(?=[A-Z0-9]))/g, '$1\n• ');
}
...
// "Soft HTML" = just <p>/<br>/<div> paragraph wrappers around what is
// otherwise plain text. No real structural tags (no <ul>, <ol>, <h*>,
// <table>). Beds24 returns Vo Rental's listings like this — a giant
// blob of <p>...</p><p>...</p> with the section headings/bullets baked
// into the prose. We strip the wrappers so the text-mode structure
// builder can do its job.
function isSoftHtmlOnly(text) {
  return /<\s*(p|br|div)\b/i.test(text)
      && !/<\s*(ul|ol|li|h[1-6]|table)\b/i.test(text);
}

function stripSoftHtmlToText(html) {
  return html
    .replace(/<\s*br\s*\/?>/gi, '\n')
    .replace(/<\s*\/\s*(p|div)\s*>/gi, '\n\n')
    .replace(/<\s*(p|div)[^>]*>/gi, '')
    .replace(/<\s*\/?(strong|b|em|i|u|a)[^>]*>/gi, '')  // inline formatting noise
...
function formatChannelDescription(input, opts = {}) {
  ...
  let sourceFormat = (opts.sourceFormat === 'html' || opts.sourceFormat === 'text')
    ? opts.sourceFormat
    : (looksLikeHtml(text) ? 'html' : 'text');

  // If the "HTML" is just paragraph wrappers (Beds24 / Airbnb-style imports),
  // strip the wrappers and re-treat as text so we can rebuild the structure
  // (section headings, bullets, mojibake-emoji handling). Real structural
  // HTML — anything with <ul>/<ol>/<h*>/<table> — is trusted as-is.
  if (sourceFormat === 'html' && isSoftHtmlOnly(text)) {
    text = stripSoftHtmlToText(text);
    sourceFormat = 'text';
  }

  const html = sourceFormat === 'html'
    ? sanitiseHtmlInput(repairMojibake(text))
    : buildStructureFromText(text);

  // outputFormat: 'html' (default — what the rich-HTML targets like the
  // contenteditable full-description field expect) or 'text' (plain text
  // for short-description inputs that can't render HTML safely). Text
  // mode strips tags but keeps line/section breaks via newlines.
  if (opts.outputFormat === 'text') {
    return html
      .replace(/<\s*\/(p|h2|h3|li|ul|br)\s*>/gi, '\n')
      .replace(/<\s*br\s*\/?>/gi, '\n')
      .replace(/<[^>]+>/g, '')
      .replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
      .replace(/\n{3,}/g, '\n\n')
