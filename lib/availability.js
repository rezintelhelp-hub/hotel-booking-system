...
// Calendar-sync sources that give TRUE state (not cascade noise). is_blocked=true
// from these sources reflects Hostfully / other PMS calendar closures and must
// close the room. The Beds24 cascade-noise workaround (which ignores CM-source
// is_blocked) remains intact for its own sources — Beds24 auto-closes sibling
// rooms when one child books, which lies about capacity.
// Steve 2026-09-15 — Hostfully sync moved to room_availability-only after
// stale synthetic 'Hostfully Block' bookings hid Hunters Gate for 34 days.
const CM_CALENDAR_SOURCES = new Set(['hostfully', 'hostfully_daily_sync']);

// Single-unit rooms have no cascade siblings, so Beds24-source is_blocked
// is truthful (owner block, external OTA booking, etc.) and must close the
// room. The cascade-noise workaround at the multi-unit branch below stays
// intact — that's where the "Beds24 auto-closes sibling when wrapper books"
// pattern actually causes false blocks. Steve 2026-09-16 — Cotswolds
// Goosewing Lakeside Lodge showed 362 days blocked in DB (source='beds24_heal')
// but rendered available because the helper filtered every Beds24 source out.
const BEDS24_CALENDAR_SOURCES = new Set([
  'beds24', 'beds24_heal', 'beds24_v1', 'beds24-v1-rates',
  'beds24-v2', 'beds24-linked', 'beds24-offers'
]);
...
async function computeRoomAvailability(pool, roomId, from, to, linkedRoomIds) {
  ...
  // Buyout-per-date lookup. Delegates to getBuyoutDates so both this
  // helper and the pool-branch endpoints share the same detection logic
  // (including cross-property linked wrappers via Beds24 dependencies).
  // Skipped when the queried room IS itself an exclusive_hire unit —
  // its own bookings already sit in bkByDate above.
  const buyoutByDate = {};
  if (propertyId && !isSelfExclusiveHire) {
    const buyoutSet = await getBuyoutDates(pool, propertyId, from, to);
    for (const d of buyoutSet) buyoutByDate[d] = 1;
  }
  ...
  for (let d = new Date(start); d < end; d.setUTCDate(d.getUTCDate() + 1)) {
    ...
    // CM calendar-sync blocks (Hostfully) — these are truthful reflections of
    // the PMS calendar, not cascade noise. Trust them for both single- and
    // multi-unit rooms. Steve 2026-09-15.
    const cmCalendarBlocked = av && av.is_blocked === true && CM_CALENDAR_SOURCES.has(av.source);
    if (cmCalendarBlocked) {
      results.push({ date: iso, available: false, units_free: 0, quantity, bookings, blocked_by: 'cm_calendar' });
      continue;
    }
    ...
    // Single-unit rooms have no cascade siblings — trust Beds24-source
    // is_blocked as an authoritative block (owner-set, OTA-booked, or
    // any other genuine closure that Beds24 reflects).
    const beds24Blocked = av && av.is_blocked === true && BEDS24_CALENDAR_SOURCES.has(av.source);
    if (beds24Blocked) {
      results.push({ date: iso, available: false, units_free: 0, quantity, bookings, blocked_by: 'cm_calendar' });
      continue;
    }

    // Single-unit: bookings-count authoritative. Any remaining is_blocked
    // with no matching booking / no recognised source is treated as
    // cascade noise (the Hebden Ex Hire multi-unit pattern doesn't affect
    // single-units, but stray non-Beds24 non-Hostfully blocks stay ignored
    // to be safe). The /public/book gate's belt-and-braces real-time
    // Beds24 check at server.js:114694 remains the last-line defence
    // against remote drift.
    if (bookings > 0) {
...
async function getBuyoutDates(pool, propertyId, from, to) {
  ...
  try {
    // "Buyout wrapper" set = exclusive_hire units that either sit on the
    // queried property OR are linked to it via the Beds24 dependency graph
    // (raw_data.dependencies.includeBookingsRoomIdN). Some properties have
    // a mirror wrapper on a separate property that operators book against
    // by mistake (Hebden 2026-09-11 — unit 2211 "IOU Hebden Bridge Hostel
    // Exclusive Hire" on property 1102 mirrors unit 1258 on property 523).
    // Beds24 knows they're paired via includeBookingsRoomId1; we honour
    // that pairing so a booking against either wrapper closes the children
    // regardless of which entry point the operator used.
    const r = await pool.query(
      `WITH my_beds24_ids AS (
         SELECT beds24_room_id::text AS bid FROM bookable_units
          WHERE property_id = $1 AND beds24_room_id IS NOT NULL
       ),
       my_exh AS (
         SELECT id, beds24_room_id::text AS bid FROM bookable_units
          WHERE property_id = $1 AND COALESCE(unit_role, 'room') = 'exclusive_hire'
       ),
       my_dep_targets AS (
         -- Beds24 room ids that any of THIS property's units reference in
         -- their includeBookingsRoomIdN dependencies. Their wrapper units
         -- (on other properties) should count as buyouts too.
         SELECT DISTINCT dep_bid FROM (
           SELECT gsrt.raw_data #>> '{dependencies,includeBookingsRoomId1}' AS dep_bid
             FROM gas_sync_room_types gsrt
             JOIN bookable_units bu ON bu.id = gsrt.gas_room_id
            WHERE bu.property_id = $1
           UNION ALL
           SELECT gsrt.raw_data #>> '{dependencies,includeBookingsRoomId2}'
             FROM gas_sync_room_types gsrt
             JOIN bookable_units bu ON bu.id = gsrt.gas_room_id
            WHERE bu.property_id = $1
           UNION ALL
           SELECT gsrt.raw_data #>> '{dependencies,includeBookingsRoomId3}'
             FROM gas_sync_room_types gsrt
             JOIN bookable_units bu ON bu.id = gsrt.gas_room_id
            WHERE bu.property_id = $1
           UNION ALL
           SELECT gsrt.raw_data #>> '{dependencies,includeBookingsRoomId4}'
             FROM gas_sync_room_types gsrt
             JOIN bookable_units bu ON bu.id = gsrt.gas_room_id
            WHERE bu.property_id = $1
           UNION ALL
           SELECT gsrt.raw_data #>> '{dependencies,includeBookingsRoomId5}'
             FROM gas_sync_room_types gsrt
             JOIN bookable_units bu ON bu.id = gsrt.gas_room_id
            WHERE bu.property_id = $1
         ) x WHERE dep_bid IS NOT NULL
       ),
       buyout_units AS (
         SELECT id FROM my_exh
         UNION
         -- exclusive_hire units whose beds24_id is referenced by any of
         -- THIS property's dependency fields (forward direction).
         SELECT DISTINCT bu.id
           FROM bookable_units bu
          WHERE COALESCE(bu.unit_role, 'room') = 'exclusive_hire'
            AND bu.beds24_room_id::text IN (SELECT dep_bid FROM my_dep_targets)
         UNION
         -- exclusive_hire units on OTHER properties that reference ONE of
         -- this property's rooms in their dependency chain (reverse
         -- direction — their bookings should still close our children).
         SELECT DISTINCT bu.id
           FROM bookable_units bu
           JOIN gas_sync_room_types gsrt ON gsrt.gas_room_id = bu.id
          WHERE COALESCE(bu.unit_role, 'room') = 'exclusive_hire'
            AND (
              (gsrt.raw_data #>> '{dependencies,includeBookingsRoomId1}') IN (SELECT bid FROM my_beds24_ids)
              OR (gsrt.raw_data #>> '{dependencies,includeBookingsRoomId2}') IN (SELECT bid FROM my_beds24_ids)
              OR (gsrt.raw_data #>> '{dependencies,includeBookingsRoomId3}') IN (SELECT bid FROM my_beds24_ids)
              OR (gsrt.raw_data #>> '{dependencies,includeBookingsRoomId4}') IN (SELECT bid FROM my_beds24_ids)
              OR (gsrt.raw_data #>> '{dependencies,includeBookingsRoomId5}') IN (SELECT bid FROM my_beds24_ids)
            )
       ),
       nights AS (
         SELECT generate_series(arrival_date, departure_date - INTERVAL '1 day', INTERVAL '1 day')::date AS night
           FROM bookings b
          WHERE b.bookable_unit_id IN (SELECT id FROM buyout_units)
