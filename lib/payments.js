...
async function recordBookingPayment(pool, bookingId, opts) {
  ...
  try {
    ...
    // INSERT the row
    // $9::text cast on both sides — Postgres otherwise deduces text on the
    // CASE side and varchar on the column side and errors out with
    // "inconsistent types deduced for parameter $9" (42P08). That silent
    // failure (my try/catch wrapper at /public/book swallowed it) skipped
    // every payment_transactions write for the ~14 Cotswolds direct
    // bookings landed 2026-09-12 evening after Phase 2 migration shipped.
    const ins = await client.query(
      `INSERT INTO payment_transactions (
         ...
         completed_at, created_at, updated_at
                 ...
                 CASE WHEN $9::text IN ('completed','succeeded') THEN NOW() ELSE NULL END,
                 NOW(), NOW())
       RETURNING id`,
